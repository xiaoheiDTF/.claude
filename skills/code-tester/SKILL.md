...
name: code-tester
description: |
  当以下条件满足时触发：需要为源码文件生成测试用例、运行测试验证代码正确性、
  code-implementer 已完成实现、用户说"写测试"、"跑测试"、"生成测试"、"/code-tester"。
  不适用：探索性代码、一次性脚本、只想看代码不改代码。
  关键词：测试、test、单元测试、接口测试、覆盖率、code-tester
...
前置: `code-implementer`（产出实现代码） → **code-tester** → 后置: `git-push`（测试通过后方可提交）
...
1. **先写失败测试再写代码（TDD 模式）** — 红-绿-重构，没有红灯的测试可能是假测试
2. **没有断言的测试不算测试** — `expect`/`assert` 是底线，没有断言的测试比没有测试更危险
3. **测试不绿不声称完成** — 证据优先于声称，"应该能通过"不算通过
4. **发现缺陷不修复源码（红线）** — 测试者只记录，不修复。修复是 code-implementer 的事
5. **测试用例只增不改（公平性红线）** — 新场景用新方法，不修改已有断言来"适配"
...
| 信号 | 含义 | 正确做法 |
|------|------|---------|
| 测试全绿但零断言 | 没有断言的测试是假通过 | 每个测试必须包含至少一个断言 |
| 测试之间有执行顺序依赖 | 测试不独立，A 失败会导致 B 也失败 | 每个测试独立设置和清理环境 |
| 只测 happy path | 边界和异常场景未覆盖 | 补充边界值、空输入、异常输入测试 |
| @MockBean 超过 5 个 | mock 太多说明测试粒度不对 | 重新评估测试范围，考虑集成测试 |
| 没有测试 Spring 上下文加载（Spring Boot 项目） | 上下文配置可能有错误 | 至少一个测试验证上下文能正常启动 |

## 借口防御

| 常见借口 | 现实 | 正确行动 |
|---------|------|---------|
| "这个函数太简单不用测" | 简单函数的 bug 最难发现，因为人们不会怀疑它 | 为所有导出函数生成测试 |
| "测试太多了，跑得慢" | 减数量不如提质量，慢的测试可能是设计问题 | 优化测试粒度，拆分慢测试 |
| "先让测试通过，断言以后再加" | 没有断言的测试 = 没有测试 | 现在就写断言，每个用例至少一个 |
| "这个 mock 太复杂了，直接用真实依赖吧" | 真实依赖让测试变慢且不稳定 | 拆分接口，让 mock 变简单 |
| "覆盖率 80% 够了，不用追 100%" | 未覆盖的 20% 往往包含边界和异常 | 检查未覆盖行，确认是否为边界/异常 |
...
#### 2.3 生成测试文件级日志（强制，每个测试文件必须生成）

每生成一个测试文件后，必须**在同一目录下**生成对应的日志文件。

**文件命名**：`<测试文件名>-<YYYYMMDD-HHmmss>.log`
- 时间戳使用本次 code-tester 启动时间（与 0.2 步骤中的启动时间一致）
- 例如：`chat.test.js-20260419-154307.log`

**日志内容必须包含**（按测试用例顺序排列）：

```
=== 测试文件: chat.test.js ===
被测源文件: src/api/chat.js
生成时间: YYYY-MM-DD HH:mm:ss
测试框架: <检测到的框架>

--- 用例 1: sendMessage 正常发送消息 ---
[输入] text: "你好", sessionId: "abc-123"
[预期] 返回 { status: "ok", messageId: "msg-001" }
[实际] 返回 { status: "ok", messageId: "msg-001" }
[结果] ✓ 通过

--- 用例 2: sendMessage 空消息抛出异常 ---
[输入] text: "", sessionId: "abc-123"
[预期] 抛出 Error("消息不能为空")
[实际] 抛出 Error("消息不能为空")
[结果] ✓ 通过
...
**格式要求**：
- 每个测试用例必须记录 `[输入]`、`[预期]`、`[实际]`、`[结果]` 四项，缺一不可
- 测试运行后（第三步），将实际运行结果追加到日志末尾的"汇总"段落
- 如果测试运行失败，将失败详情追加到对应用例的 `[结果]` 行下方
- 使用 `Write` 工具生成，后续步骤中用 `Edit` 追加运行结果

**Spring Boot 项目特殊处理**：
- 不生成 `run-tests.sh` / `run-tests.ps1`，但**仍然生成测试文件级日志**
- 日志文件放在 `src/test/java/com/example/` 对应目录下（与测试文件同目录）
- 例如：`src/test/java/com/example/UserServiceImplTest.java-20260419-154307.log`
...
6. **将运行结果追加到测试文件级日志**（2.3 步生成的 `<测试文件名>-YYYYMMDD-HHmmss.log`），更新"汇总"段落中的通过/失败数和失败详情
...
└── tests/                         ← 统一的测试根目录
    ├── api/
    │   ├── chat.test.js           ← 主测试文件
    │   ├── chat.test.js-YYYYMMDD-HHmmss.log  ← 测试文件级日志（输入/输出/预期/实际）
    ...
    └── components/
        ...
        ├── ChatView.test.vue-YYYYMMDD-HHmmss.log  ← 测试文件级日志
...
已生成以下文件：
  ...
  测试文件级日志（输入/输出/预期/实际）:
  - tests/api/chat.test.js-20260419-154307.log
  - tests/components/ChatView.test.vue-20260419-154307.log
...
26. **测试用例只增不改（公平性红线）** — 新写的测试用例只可以去添加，不可以修改已有测试。具体包括：① 新增场景时用新的 @Test 方法，不改动已有方法 ② 断言失败时先查根因，不直接修改断言值 ③ 需要修改已有测试时必须向用户说明原因并获得确认 ④ 用 @Nested 分组组织不同场景的测试，保持各组独立
27. **测试文件级日志强制生成** — 每个测试文件生成后，必须在**同目录下**生成 `<测试文件名>-YYYYMMDD-HHmmss.log`，内容包含每个用例的输入、预期、实际、结果。测试运行后必须将实际运行结果追加到日志的"汇总"段落
