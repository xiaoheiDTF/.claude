...
镜像位置：.claude/module-test/frontend/src/core/browser/cdp/command/test/selector.test.ts
```
...
|---------|---------------|---------------------|
| `travel-agent/app/models/chat.py` | `travel-agent/app/models/test/test_chat.py` | `.claude/module-test/travel-agent/app/models/test/test_chat.py` |
| `travel-web/src/api/chat.js` | `travel-web/src/api/test/chat.test.js` | `.claude/module-test/travel-web/src/api/test/chat.test.js` |
| `travel-web/src/components/ChatView.vue` | `travel-web/src/components/test/ChatView.test.vue` | `.claude/module-test/travel-web/src/components/test/ChatView.test.vue` |

**同步要求**：
- 每次生成测试文件时，必须**同时**写入两个位置：
  1. **主位置**：源码目录下的 `test/` 子目录
  2. **镜像位置**：`.claude/module-test/` 下与代码目录结构完全一致的路径
- `.claude/module-test/` 是测试文件的集中管理入口，方便后期统一检索和维护

**框架特殊约定时的处理**：
- 如果框架强制要求特定测试目录（如 Spring Boot 的 `src/test/java/`），测试文件先生成在框架要求位置
- 然后**复制一份**到 `.claude/module-test/` 的对应镜像路径，保持树形结构一致
...
**特别约束**：
1. 测试文件必须同时写入**主位置**（源码目录的 `test/` 子目录）和**镜像位置**（`.claude/module-test/` 对应路径）
2. `skill-gate` Hook 会在每次测试文件写入后自动检查镜像是否同步
3. 配套文件生成后必须立即运行 `check-deliverables.sh`
4. `tester-logs/` 日志必须按本次启动时间生成，不可遗漏

测试文件统一放在各模块目录的 `test/` 子目录下，并同步镜像到 `.claude/module-test/`。配套文件（运行脚本、说明文档、日志）统一放在**子项目根目录**下：
...
|--------|-----------|---------|---------|-------|
| api/chat.js | api/test/chat.test.js | .claude/module-test/.../api/test/chat.test.js | sendMessage, sendMessageStream | 8 |
...
将测试结果、`tester-logs/` 执行日志、以及 `.claude/module-test/` 镜像快照同步到 `doc/ai-coding` 流水线目录，纳入 `05-test/` 阶段：
...
5. `.claude/module-test/` 中本次涉及模块的镜像路径生成快照索引 `module-test-index.json`，放入 `05-test/`
6. 该阶段文档生成后，才算测试交付完成
...
19. **module-test 镜像强制同步（红线）** — 每次生成或修改测试文件后，**必须**同步到 `.claude/module-test/` 的对应镜像路径。主位置和镜像位置的内容必须保持一致。镜像路径的目录结构必须与代码目录完全一致。`skill-gate` Hook 会自动检查此项，遗漏会触发验证警告
