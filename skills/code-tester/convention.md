...
> 测试文件统一放在 `<subproject>/tests/` 下，按源文件目录结构镜像组织，不散落在源码目录旁边。
...
测试文件路径:
  <subproject>/tests/a/b/c/module.test.ext
```

**选择策略**：
1. 如果该子项目已有测试文件 → 参考已有测试的放置方式
2. 框架有明确约定 → 遵循框架约定（见 3.2）
3. 都没有 → 使用 `<subproject>/tests/` 集中镜像结构
...
|------|------|-----------|------------|---------|---------|
| Java | Spring Boot Test | `src/main/java/**/*Service.java` | `src/test/java/**/*ServiceTest.java` | `src/test/java/` 镜像（已集中） | `@SpringBootTest`、`@WebMvcTest`、`@MockBean` |
| Java | JUnit 5 | `src/main/java/**/*.java` | `src/test/java/**/*Test.java` | `src/test/java/` 镜像（已集中） | `@Nested` 分组、`@ParameterizedTest` |
| Java | TestNG | `src/main/java/**/*.java` | `src/test/java/**/*Test.java` | `src/test/java/` 镜像（已集中） | `@DataProvider` 参数化 |
| Python | pytest | `**/*.py` | `tests/**/test_*.py` | `tests/` 镜像 | `conftest.py` fixture、`@pytest.mark.parametrize` |
| Python | unittest | `**/*.py` | `tests/**/test_*.py` | `tests/` 镜像 | `class Test*(TestCase)` |
| Python | Django Test | `**/views.py` | `tests/**/tests.py` 或 `tests/**/test_*.py` | `tests/` 镜像 | `TestCase`、`Client`、`factory_boy` |
| Go | testing | `**/*.go` | `**/*_test.go` | 同目录（语言强制） | 同包测试（白盒）+ `_test` 包（黑盒） |
| Go | testify | `**/*.go` | `**/*_test.go` | 同目录（语言强制） | `suite.TestSuite` |
| Rust | cargo test | `src/**/*.rs` | 内联 `#[test]` 或 `tests/*.rs` | `tests/`（已集中） | `#[cfg(test)]` 模块 |
| C# | xUnit | `**/*.cs` | `Tests/**/*Tests.cs` | `Tests/` 镜像（已集中） | `[Fact]`、`[Theory]`、`[InlineData]` |
| C# | NUnit | `**/*.cs` | `Tests/**/*Tests.cs` | `Tests/` 镜像（已集中） | `[Test]`、`[TestCase]` |
| Ruby | RSpec | `lib/**/*.rb` | `spec/**/*_spec.rb` | `spec/` 镜像（已集中） | `describe/context/it` 嵌套 |
| PHP | PHPUnit | `src/**/*.php` | `tests/**/*Test.php` | `tests/` 镜像（已集中） | `extends TestCase` |
| JS/TS | Vitest | `src/**/*.ts` | `tests/**/*.test.ts` | `tests/` 镜像（优先集中） | `describe/it/expect` |
| JS/TS | Jest | 同上 | `tests/**/*.test.ts` | `tests/` 镜像（优先集中） | 全局 API |
| JS/TS | Mocha | 同上 | `tests/**/*.test.ts` | `tests/` 镜像（优先集中） | `describe/it` + chai/assert |
...
每次为某个目录生成测试时，在该目录对应的**集中测试目录**下必须生成以下文件。集中测试目录统一为 `<subproject>/tests/<相对源码路径>/`：

```
<subproject>/tests/api/          ← 对应 src/api/ 的集中测试目录
├── chat.test.js                 ← 测试文件（含预期/实际打印）
├── user.test.js
├── run-tests.sh                 ← 一键运行脚本（只跑当前目录下的测试）
├── run-tests.ps1                ← Windows 版本
...
**关键规则**：
- 测试文件统一放在 `<subproject>/tests/` 下，不散落在 `src/` 源码目录旁边
- 测试目录与源码目录保持**镜像结构**：`src/api/chat.js` ↔ `tests/api/chat.test.js`
- 每个测试子目录（如 `tests/api/`、`tests/models/`）都是**自包含的测试包**，有自己的脚本、报告和日志
- `run-tests.sh` / `run-tests.ps1` 的测试范围**仅限当前目录**（如 `tests/api/`），不越界跑其他模块的测试
...
- 所有输出同时写入日志文件（`logs/YYYYMMDD-HHmmss.log`）
- 日志文件保存在测试目录下的 `logs/` 子目录
- 显示测试开始/结束时间、耗时、退出码
...
**Unix / Git Bash**:
  bash tests/<模块路径>/run-tests.sh              # 运行
  bash tests/<模块路径>/run-tests.sh --verbose    # 详细输出
  bash tests/<模块路径>/run-tests.sh --coverage   # 覆盖率

**Windows PowerShell**:
  pwsh tests/<模块路径>/run-tests.ps1             # 运行
  pwsh tests/<模块路径>/run-tests.ps1 -Verbose    # 详细输出
  pwsh tests/<模块路径>/run-tests.ps1 -Coverage   # 覆盖率

## 日志文件

每次运行自动生成日志到 `tests/<模块路径>/logs/` 目录，文件名格式 `YYYYMMDD-HHmmss.log`。
...
| 测试路径参数 | `src/core/browser/.../tests/` | `core/browser/.../tests/`（去掉 `src/` 前缀） |
| `tests/` 到根的相对层级 | 按 `package.json` 目录计算 | 按 Vitest 根目录计算（少一层） |
...
测试目录: frontend/src/core/browser/cdp/command/tests/
tests/ → Vitest 根的相对路径: ../../../../../ (5 级)
测试路径参数: core/browser/cdp/command/tests/  (不含 src/)
```
