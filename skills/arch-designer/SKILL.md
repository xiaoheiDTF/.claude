...
name: arch-designer
description: |
  当以下条件满足时触发：需要设计系统架构、进行技术选型、模块划分、定义接口契约、
  用户说"设计架构"、"架构设计"、"模块划分"、"技术选型"、"/arch-designer"。
  不适用：具体编码实现、只需要改几行代码、已有架构只需微调。
  关键词：架构、模块、分层、技术选型、接口契约、架构设计、arch-designer
...
前置: `idea-to-req`（需求） → `task-breakdown`（拆解） → **arch-designer** → 后置: `struct-designer`（结构设计） → `impl-planner`（执行计划）
...
1. **每个架构决策必须有取舍分析** — 没有取舍的决策是拍脑袋，ADR 不只是记录结论，更要记录被否决的方案和否决理由
2. **有约束才设计** — 没有性能约束不设计缓存，没有扩展需求不设计 SPI。需求驱动设计，不是设计驱动需求
3. **不做过度设计** — YAGNI，不为"以后可能需要"设计复杂架构，架构复杂度必须匹配需求复杂度
4. **能复用现有架构就不新建** — 项目已有模块能满足需求时直接映射，不另起炉灶
5. **依赖方向必须单向** — 从高层指向低层，严禁循环依赖，有循环说明模块划分有问题
...
| 信号 | 含义 | 正确做法 |
|------|------|---------|
| 设计了 3 层以上抽象 | 过度设计，复杂度超出需求 | 减少抽象层级，回到需求本身 |
| 忽略非功能需求 | 性能、安全、可靠性可能出问题 | 补充非功能需求分析 |
| 没有替代方案对比 | 技术选型缺乏依据 | 每个选型至少列出一个替代方案和否决理由 |
| 模块职责说不清 | 职责边界模糊，后续实现会混乱 | 重新划分模块直到一句话能说清职责 |
| 架构无法映射到项目路径 | 设计是空中楼阁，无法落地 | 根据项目实际目录结构调整设计 |

## 借口防御

| 常见借口 | 现实 | 正确行动 |
|---------|------|---------|
| "先设计完美再开始" | 完美是迭代出来的，不是一次设计出来的 | 设计到够用就交付，后续迭代优化 |
| "以后可能需要这个扩展点" | YAGNI，未来需求的不确定性让提前设计变成技术债 | 等需求真的来了再加 |
| "这个选型大家都这么用" | "大家都用"不是理由，你的场景可能不同 | 写出具体的项目约束和选型理由 |
| "架构图先不画了，太麻烦" | 没有图的架构等于没有架构 | 画出 Mermaid 图，这是交付物的一部分 |
