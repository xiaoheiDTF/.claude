...
当"修复模式"下扫描发现需要处理的目录超过 **10 个**时，必须使用 `TodoWrite` 按操作类型分组创建任务列表：

1. 生成缺失的 CLAUDE.md
2. 修复过时的 CLAUDE.md
3. 补全不完整的 CLAUDE.md
4. 同步 module-registry 镜像
5. 更新 index.json

每完成一个分组，更新 Todo 状态。避免在大量目录变更时遗漏或失去进度掌控。
...
8. **大规模修复用 Todo 追踪** — 当需要处理的目录超过 10 个时，使用 TodoWrite 分步追踪进度（生成缺失 → 修复过时 → 补全不完整 → 同步 registry → 更新索引），避免遗漏
