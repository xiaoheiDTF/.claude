...
Strip away the CLI polish, progress bars, permission systems, and what remains is surprisingly simple: a loop that lets the model continuously call tools until the task is complete.
...
Claude Code has about 20 tools, but 4 cover 90% of scenarios:

| Tool | Purpose | Example |
| --- | --- | --- |
| `bash` | Run commands | `npm install`, `git status` |
| `read_file` | Read content | View `src/index.ts` |
| `write_file` | Create/overwrite | Create `README.md` |
| `edit_file` | Surgical edits | Replace a function |
...
=========================================
What is the core pattern behind this generation of agent coding tools? There is no secret.
...
- Model controls the loop (keeps calling tools until stop_reason != "tool_use")
- Results become context (fed back as “user” messages)
- Memory accumulates automatically (messages list saves history)
## System Prompt
...
Model decides which tools to use, in what order, when to stop.

**3. Transparent**
Every tool call visible, every result in the conversation.

**4. Extensible**
Adding a tool = one function + one JSON schema.
...
Key point: **The core is tiny**, everything else is refinement.

## Bigger Picture

This generation of agent coding systems shares the same pattern:
...
Differences are in tools, display, safety. But the essence is always: **give the model tools, let it work**.
...
---
**Model as Agent. That’s the entire secret.**
[v0: Bash is EverythingOne tool + loop, full Agent capability
](../v0-bash-agent/)[v2: Explicit TodoMake plans a visible state machine, reduce going off-track
