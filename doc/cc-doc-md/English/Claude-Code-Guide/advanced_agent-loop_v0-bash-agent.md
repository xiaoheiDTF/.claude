...
# v0: Bash is Everything

After building v1, v2, and v3, a question emerged: What is the **essence** of an Agent?

v0 answers this through reverse thinking—strip everything away until only the core remains.
...
The last line is the key insight: **calling itself via bash implements subagents**. No Task tool needed, no Agent Registry—just recursion.
...
Main Agent
  └─ bash: python v0_bash_agent.py "analyze architecture"
       └─ Subagent (separate process, fresh history)
            ├─ bash: find . -name "*.py"
...
- Subprocess has its own history=[]
- Parent process captures stdout as tool result
- Recursive calls enable infinite nesting
## What v0 Sacrifices

| Feature | v0 | v3 |
| --- | --- | --- |
| Agent types | None | explore/code/plan |
| Tool filtering | None | Whitelist |
...
# Optional: Configure custom endpoint (uses official API by default)

# Interactive mode
python v0_bash_agent.py

# Subagent mode
python v0_bash_agent.py "Analyze current directory structure and summarize"
```

## Common Questions
**Q: Why not just use v1 directly?**
A: v0 is good for building intuition. Understanding that “one tool can work too” makes the tool separation in v1 more appreciable.

**Q: Won’t subagents waste tokens?**
A: No. Subagents run independently and only return summaries to the parent. This actually reduces noise in the main context.

**Q: Can I use v0 in production?**
A: Not recommended. Lacks safety guardrails, permission systems, progress display, etc. v0 is a minimal implementation for teaching.
