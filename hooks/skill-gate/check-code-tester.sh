...
if echo "$PATH" | grep -qE "/test/(test_|.*\.test\.)"; then
  # Infer the mirror path in doc/module-test/
  PROJECT_DIR="$(cd "$(dirname "$0")/../../.." && pwd)"
  REL_PATH=$(echo "$PATH" | sed "s|$PROJECT_DIR/||")
  MIRROR_PATH="$PROJECT_DIR/doc/module-test/$REL_PATH"

  if [ ! -f "$MIRROR_PATH" ]; then
    echo "{\"additionalContext\": \"[skill-gate] 检测到测试文件 $REL_PATH 已写入，但未在 doc/module-test/ 中找到镜像备份。请立即运行 module-test 同步。\"}"
    exit 1
  fi
fi
