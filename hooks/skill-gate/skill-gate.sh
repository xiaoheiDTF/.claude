...
IS_SRC_PATH=$(echo "$FILE_PATH" | grep -cE "(^|.*/)(src|lib|app|frontend|packages|services|modules)/" 2>/dev/null || echo 0)

if echo "$FILE_PATH" | grep -qE "(impl-plans|03-plan)"; then
...
if [ "$IS_SRC_PATH" -gt 0 ] && echo "$FILE_PATH" | grep -qE "(__tests__|tests/)"; then
...
if echo "$FILE_PATH" | grep -qE "04-report"; then
