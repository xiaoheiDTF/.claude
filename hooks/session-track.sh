...
echo "${TIMESTAMP}|${TOOL}|${FILE_PATH}" >> "$TRACK_DIR/${SESSION_ID}.track"

# Keep file size reasonable (last 500 entries only)
if [ -f "$TRACK_DIR/${SESSION_ID}.track" ]; then
  LINES=$(wc -l < "$TRACK_DIR/${SESSION_ID}.track")
  if [ "$LINES" -gt 500 ]; then
    tail -500 "$TRACK_DIR/${SESSION_ID}.track" > "$TRACK_DIR/${SESSION_ID}.track.tmp"
    mv "$TRACK_DIR/${SESSION_ID}.track.tmp" "$TRACK_DIR/${SESSION_ID}.track"
  fi
fi
